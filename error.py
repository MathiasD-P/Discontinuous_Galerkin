...
order = 2
alpha = 0
bcs = lambda t : np.array([0.,1]) # could be a function of time
ics = lambda x : 0.5 * (1 + np.tanh(250*(x - 20))) # function of space only
...
bcs = lambda t : ics(np.array([0,40.]) - A * (t - 1 / (N-1))) # could be a function of time
...
for j in range(0,len(J)):
    ...
    u_exact = lambda x : exact_solution(x, t[N-1])
    
    error[j] = DG.L2(u, u_exact, problem, 10)
...
plt.legend(["RMS Error (logarithmic)", "Least Squares Fit (slope =" + str(round(fit[0],2)) + ")", "Fit Interval"])
